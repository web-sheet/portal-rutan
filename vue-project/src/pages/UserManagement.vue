  ...
  <div class="p-6 max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center border-b border-slate-200 pb-4">
      <div>
        ...
        <p class="text-sm text-slate-500">Kelola hak akses akun mulai dari Kaur, Kasi, Karutan, hingga Admin Aplikasi.</p>
      </div>
      <Button 
    ...
    <div class="card bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
      <DataTable :value="users" stripe responsiveLayout="scroll" class="p-datatable-sm text-sm">
        ...
        <Column field="role" header="Role Hak Akses" sortable>
          <template #body="slotProps">
            <Tag :value="slotProps.data.role" :severity="getRoleSeverity(slotProps.data.role)" class="uppercase font-bold" />
          </template>
        </Column>
    ...
    <Dialog 
      ...
      :header="isEditMode ? '⚙️ Edit Data Petugas' : '👤 Tambah Petugas Baru'" 
      :modal="true" 
      :style="{ width: '450px' }"
      class="p-fluid"
    >
      <form @submit.prevent="handleSubmit" class="space-y-4 mt-2">
        ...
        <div class="flex justify-end gap-2 pt-4 border-t border-slate-100">
          ...
          <Button type="submit" :label="isEditMode ? 'Simpan' : 'Daftarkan'" icon="pi pi-check" :loading="loadingSubmit" severity="success" />
        </div>
      </form>
    </Dialog>
  </div>
